    ...
    def make_request(self, messages: List[ChatCompletionMessageParam]) -> str:
        ...
        for i in range(num_attempts):  
            try:
                ...
                LLMServiceFactory.update_tokens_usage(self.model_name, response.usage.prompt_tokens, response.usage.completion_tokens)
...
class LLMServiceFactory():
    ...
    @classmethod
    def get_service(cls, model_name: str, provider: str, temperature: float = 0, max_tokens: int = 512) -> LLMService:
        key = (model_name, provider, temperature, max_tokens)
        if key not in cls._services:
            # Check if the model is supported
            if model_name not in cls.SUPPORTED_MODELS:
                raise ValueError(f"Model not found in the list of supported models: {cls.SUPPORTED_MODELS}.")
            if provider == "AzureOpenAI":
                ...
                raise ValueError(f"Unsupported provider: {provider}. Supported providers: AzureOpenAI, HuggingFace")
    ...
    def update_tokens_usage(cls, model_name, prompt_tokens, completion_tokens):
        ...
        try:
            ...
            if not COST_RATES.get(model_name):
                ...
                COST_RATES[model_name] = {"prompt_token_cost": 0, "completion_token_cost": 0}
            prompt_cost = prompt_tokens * COST_RATES[model_name]["prompt_token_cost"]
            completion_cost = completion_tokens * COST_RATES[model_name]["completion_token_cost"]
